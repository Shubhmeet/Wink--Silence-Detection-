...
bool detectWink(Mat frame, Point location, Mat ROI, CascadeClassifier cascade) {
	...
	vector<Rect> eyes;
	cascade.detectMultiScale(ROI, eyes, 1.1, 3, 0, Size(10, 10));
...
int detect(Mat frame,
	...
	*/
	...
	if (nfaces == 0) {
		vector<Rect> eyes;
		cascade_eyes.detectMultiScale(frame, eyes, 1.18, 3, 0, Size(34,34));
		int neyes = (int)eyes.size();
		for (int i = 0; i < neyes; i++) {
			Rect eyes_i = eyes[i];
			drawEllipse(frame, eyes_i, 255, 255, 0);
	...
	for (int i = 0; i < nfaces; i++) {
		...
		int x1 = face.x;
		int y1 = face.y;
		Rect eye = Rect(x1, y1, face.width, (face.height / 2)+1);
		Mat eye_ROI = frame_gray(eye);
		drawEllipse(frame, eye, 100, 0, 255);
		if (detectWink(frame, Point(face.x, face.y), eye_ROI, cascade_eyes)) {
